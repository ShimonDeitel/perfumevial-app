...
struct ContentView: View {
    ...
    private func subtitle(for item: Bottle) -> String {
        var parts: [String] = []
        
        parts.append("Notes: " + describeField(item.notes))
        parts.append("FillLevel: " + describeField(item.fillLevel))
        parts.append("Occasion: " + describeField(item.occasion))
...
struct ItemFormView: View {
    ...
    init(mode: FormMode) {
        ...
        switch mode {
        case .add:
            _draft = State(initialValue: Bottle(id: UUID(), title: String = "", notes: String = "", fillLevel: Double = 0, occasion: String = ""))
        case .edit(let item):
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                TextField("FillLevel", value: $draft.fillLevel, format: .number)
                    .keyboardType(.decimalPad)
                    .accessibilityIdentifier("field_fillLevel")
                TextField("Occasion", text: $draft.occasion)
                    .accessibilityIdentifier("field_occasion")
            }
